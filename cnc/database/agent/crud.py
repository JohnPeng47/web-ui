...
async def get_agent_by_id(db: AsyncSession, agent_id: str) -> Optional[Union[DiscoveryAgent, ExploitAgent]]:
    ...
    result = await db.execute(select(ExploitAgent).where(ExploitAgent.id == agent_id))
...
) -> Union[DiscoveryAgent, ExploitAgent]:
    agent = await get_agent_by_id(db, agent_id)
    if not agent:
        raise ValueError(f"Agent with ID {agent_id} not found")
