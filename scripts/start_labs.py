...
PORTSWIGGER_CREDS = {
    ...
    "password": "R4KtdF[8?dA5qXFLy^ddAYKPX94qs')3",
}
PORTSWIGGER_URL = "https://portswigger.net"
DATA_DIR_PATH = Path("tmp/profile s").resolve()
...
class PortSwiggerLabRunner:
    ...
    def __init__(
        ...
        self._failed_labs: Dict[Tuple[str, int], str] = {}  # Track failures
    ...
    async def _async_run(self) -> None:
        async with aiohttp.ClientSession() as session:
            # Start all labs and collect polling tasks
            for spec in self._labs_to_start:
                ...
                for lab_idx in spec["labs"]:
                    ...
                    if not url:
                        self._failed_labs[key] = "Failed to start or capture URL"
                        logger.warning("⚠️  Skipping polling for failed lab: %s/#%s", vuln, lab_idx)
                    ...
                    # Give polling task a chance to start
                    await asyncio.sleep(0)

            # Print summary
            logger.info("=" * 60)
            logger.info("Lab startup summary:")
            logger.info("  ✅ Successfully started: %d labs", len(self._active_labs))
            logger.info("  ❌ Failed to start: %d labs", len(self._failed_labs))
            if self._failed_labs:
                for (vuln, idx), reason in self._failed_labs.items():
                    logger.info("    - %s/#%s: %s", vuln, idx, reason)
            logger.info("=" * 60)

            # All labs requested; keep polling forever (or until cancelled).
            if self._poll_tasks:
                logger.info("Starting continuous polling for %d active labs...", len(self._poll_tasks))
                await asyncio.gather(*self._poll_tasks)
            else:
                logger.error("No labs started successfully. Nothing to poll. Exiting.")
